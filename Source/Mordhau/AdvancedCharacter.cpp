...
//	class UMordhauCapsuleComponent* MordhauCapsuleComponent = CreateDefaultSubobject<UMordhauCapsuleComponent>(TEXT("CapsuleComponent"));
//	UPROPERTY(BlueprintReadWrite, VisibleAnywhere, meta = (AllowPrivateAccess = "true"))
//	class USkeletalMeshComponent* SkeletalMeshComponent = CreateDefaultSubobject<USkeletalMeshComponent>(TEXT("Mesh"));
//	SkeletalMeshComponent->bEditableWhenInherited = true;
//	SkeletalMeshComponent->AttachTo(MordhauCapsuleComponent);
//}
AAdvancedCharacter::AAdvancedCharacter(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer
		.SetDefaultSubobjectClass<UAdvancedCharacterMovement>(ACharacter::CharacterMovementComponentName)
...
float AAdvancedCharacter::ModifyDamage_Implementation(float DamageAmount, EMordhauDamageType DamageType, struct FHitResult HitInfo, class AActor* Source, class AActor* Agent, class AController* EventInstigator)
{
	return 1;
}
bool AAdvancedCharacter::ModifyParryResult_Implementation(bool InResult, struct FHitResult HitResult, class AMordhauCharacter* Attacker)
{
	return 1;
}
